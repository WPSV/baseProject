...
export default function DataUpload() {
    return (
        <div className="p-6 bg-light-100 rounded-lg w-[300px]">
            <h1 className="text-xl font-bold text-secondary-400 mb-6">Histórico</h1>
            {data.map((group, index) => (
                <div key={index} className="mb-6">
                    <div className="font-bold text-secondary-400 text-sm mb-2">{group.date}</div>
                    {group.items.map((item, idx) => (
                        <div key={idx} className="grid grid-cols-[auto_1fr_auto] gap-4 items-center mb-4">
                            ...
                            <div className="text-sm line-clamp-2 text-secondary-400">{item.name}</div>
