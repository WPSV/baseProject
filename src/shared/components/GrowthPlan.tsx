...
export default function GrowthPlan({ data }: GrowthPlanProps) {
    ...
    if (!data || data.length === 0) {
        return (
            <div className="text-colorTitle bg-secondary rounded-lg p-6 relative">
    ...
    return (
        <div className="w-full text-colorTitle p-6 rounded-lg bg-secondary h-72 flex flex-col">
            <h2 className="text-xl font-bold mb-5 sticky top-0 bg-secondary z-10">Planos de crescimento</h2>
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <div key={index} className="mb-8 flex items-center relative">
                            ...
                            <div className={`ml-8 flex-grow bg-colorBgField p-4 rounded-lg flex items-center relative ${!isFirstOfGroup && 'ml-32'}`}>
